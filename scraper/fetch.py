    ...
    def download_and_process(self, url):
        ...
        try:
            ...
            if r.status_code != 200:
                print(f"[!] ERROR: Could not download DBF. Status {r.status_code}")
                return
            ...
            for record in table:
                ...
                count += 1
            print(f"[+] SUCCESS: Loaded {count} property owners.")
    ...
    async with async_playwright() as p:
        ...
        for doc_label, doc_code in DOC_TYPES.items():
            ...
            try:
                ...
                try:
                    # We wait a shorter time here to trigger the screenshot faster if it fails
                    await page.wait_for_selector('select[name="doc_type"]', timeout=10000)
                except:
                    print("TIMEOUT/BLOCK. Taking diagnostic screenshot...")
                    # SAVE SCREENSHOT TO REPO SO WE CAN SEE WHAT THE BOT SEES
                    await page.screenshot(path="diagnostic_error.png")
                    print("[!] Screenshot saved as diagnostic_error.png")
                    # If the first one failed, they all will. Stop here to save time.
                    break 
                ...
                for row in rows:
                    ...
                    rec = {
                        "doc_num": cols[0].text.strip(), "doc_type": doc_label, "filed": cols[1].text.strip(),
                        "cat": doc_code, "cat_label": doc_label, "owner": owner_name, "grantee": cols[3].text.strip(),
                        "amount": cols[4].text.strip() if len(cols)>4 else "0", "legal": cols[5].text.strip() if len(cols)>5 else "",
                        "prop_address": address_data.get("prop_addr", ""), "prop_city": address_data.get("prop_city", ""),
                        "prop_state": address_data.get("prop_state", "MO"), "prop_zip": address_data.get("prop_zip", ""),
                        "mail_address": address_data.get("mail_addr", ""), "mail_city": address_data.get("mail_city", ""),
                        "mail_state": address_data.get("mail_state", "MO"), "mail_zip": address_data.get("mail_zip", ""),
                        "clerk_url": page.url,
                    }
...
async def main():
    print("[*] Starting FlowX Diagnostic Engine...")
    records = await scrape_clerk()
    output = {
        "fetched_at": datetime.now().isoformat(), "source": "Greene County", "date_range": f"{LOOKBACK_DAYS} days",
        "total": len(records), "with_address": len([r for r in records if r['mail_address']]), "records": records
    }
    ...
    export_ghl(records)
    print(f"[+] FINAL RESULT: {len(records)} leads saved.")
